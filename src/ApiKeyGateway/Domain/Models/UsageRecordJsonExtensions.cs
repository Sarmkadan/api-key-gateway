...
public static class UsageRecordJsonExtensions
{
    ...
    /// <summary>
    /// Serializes a <see cref="UsageRecord"/> to a JSON string
    /// </summary>
    ...
    /// <returns>The deserialized usage record, or null if the JSON is null or empty</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="json"/> is null</exception>
    /// <exception cref="JsonException">Thrown when the JSON is invalid or cannot be deserialized</exception>
    public static UsageRecord? FromJson(string json)
    {
        ArgumentNullException.ThrowIfNull(json);
        return json.Length == 0 ? null : JsonSerializer.Deserialize<UsageRecord>(json, _jsonOptions);
    }
    ...
    public static bool TryFromJson(string json, out UsageRecord? value)
    {
        ...
        try
        {
            value = json.Length == 0 ? null : JsonSerializer.Deserialize<UsageRecord>(json, _jsonOptions);
            return true;
        }
        catch (JsonException)
        {
