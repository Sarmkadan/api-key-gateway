...
public static class CorrelationContextMiddlewareValidation
{
    ...
    public static IReadOnlyList<string> Validate(this CorrelationContextMiddleware value)
    {
        ...
        // Validate injected RequestDelegate
        // RequestDelegate is a framework delegate that should never be null when properly constructed
        if (value.GetNextDelegate() is null)
        {
            errors.Add("The RequestDelegate (next middleware) is null.");
        }

        // Validate injected ILogger
        // ILogger should be injected by the framework and should not be null
        if (value.GetLogger() is null)
        {
            errors.Add("The ILogger instance is null.");
        }
    ...
    /// <exception cref="ArgumentNullException">Thrown if <paramref name="value"/> is null.</exception>
    public static bool IsValid(this CorrelationContextMiddleware value) => Validate(value).Count == 0;
