...
/// <summary>
/// Provides JSON serialization helpers for <see cref="ApiKeyModelTests"/> instances.
/// </summary>
public static class ApiKeyModelTestsJsonExtensions
{
	/// <summary>
	/// Default JSON serialization options configured for camelCase property naming.
	/// </summary>
	...
	public static ApiKeyModelTests FromJson(string json)
	{
		...
		return JsonSerializer.Deserialize<ApiKeyModelTests>(json, DefaultOptions)
		?? throw new JsonException("Failed to deserialize ApiKeyModelTests from JSON.");
	}
	...
	public static bool TryFromJson(string json, out ApiKeyModelTests? value)
	{
		...
		try
		{
			value = JsonSerializer.Deserialize<ApiKeyModelTests>(json, DefaultOptions);
